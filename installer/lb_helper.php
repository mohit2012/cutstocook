...
class LicenseBoxAPI{
	...
	public function __construct(){
		$this->product_id = '71A37897';
		$this->api_url = 'https://ondemand.licencefly.com/';
		$this->api_key = '9DC8C2734C2B3-foodeli';
	...
	public function verify_license($time_based_check = false, $license = false, $client = false){
		...
		if($time_based_check && $this->verification_period > 0){
			...
			$today = date('d-m-Y');
			if(empty($_SESSION["543f50ea7edef0f"])){
				$_SESSION["543f50ea7edef0f"] = '00-00-0000';
			}
			if($type == 1){
				$type_text = '1 day';
			}elseif($type == 3){
			...
			if(strtotime($today) >= strtotime($_SESSION["543f50ea7edef0f"])){
				$get_data = $this->call_api(
					'POST',
